...
def get_usage(user_id: int, year_month: str | None = None) -> dict:
    ...
    conn = get_db()
    row = conn.execute(
        "SELECT files_processed, rows_processed FROM usage_logs WHERE user_id=? AND year_month=?",
        (user_id, ym),
    ).fetchone()
    if row:
        return {"files_processed": row["files_processed"], "rows_processed": row["rows_processed"], "year_month": ym}
...
def get_all_time_usage(user_id: int) -> dict:
    ...
    conn = get_db()
    row = conn.execute(
        "SELECT SUM(files_processed) as tf, SUM(rows_processed) as tr FROM usage_logs WHERE user_id=?",
        (user_id,),
    ).fetchone()
...
def get_platform_usage() -> list[dict]:
    ...
    rows = conn.execute(
        ...
        "GROUP BY u.id ORDER BY total_files DESC NULLS LAST"
    ).fetchall()
